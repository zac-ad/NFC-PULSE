...
// Sits on top of the profile — does not unmount or replace any layout.
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md p-6">
      <div className="w-full max-w-xs bg-neutral-950 border border-neutral-800/80 rounded-3xl p-8 text-center space-y-5 shadow-2xl">
        <div className="w-10 h-10 rounded-full border border-neutral-700 bg-neutral-900 flex items-center justify-center mx-auto">
        ...
        <div className="bg-neutral-950/90 border border-neutral-800/80 rounded-3xl overflow-hidden shadow-[0_10px_40px_rgba(0,0,0,0.8)] backdrop-blur-xl relative text-center">
          ...
          <div className="px-6 pb-6 pt-0 relative flex flex-col items-center">
            ...
            {connected && (
              ...
              {profile.phone && (
                <a href={`sms:${profile.phone}`} title="Send Message"
                  className="w-11 h-11 rounded-full bg-neutral-900/90 border border-neutral-800 hover:bg-neutral-800 flex items-center justify-center text-white transition-all shadow-md active:scale-95 hover:border-neutral-700">
                  <svg className="w-4 h-4 fill-current text-neutral-300" viewBox="0 0 24 24">
          ...
          <div className="space-y-2">
            ...
            <div className="space-y-2">
              {socialLinks.map((link) => (
                <a key={link.id} href={resolveDeepLink(link.url)} target="_blank" rel="noreferrer"
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className="font-semibold text-sm text-neutral-200 group-hover:text-white transition-colors">{link.title}</span>
                  </div>
...
function PublicProfilePageInner() {
  ...
  // Refs for debouncing activity pings
  ...
  useEffect(() => {
    ...
    // 5-minute recurring ping to refresh timer and re-check card status
    pingIntervalRef.current = setInterval(pingSession, 5 * 60 * 1000);

    // Activity listeners — debounced in handleActivity
