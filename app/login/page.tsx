...
function LoginForm() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      setMessage({
        type: 'success',
        text: `Login link sent to ${email}! Check your inbox to open your command dashboard.`,
      });
    }
    setLoading(false);
  };
    ...
    <div className="max-w-md w-full bg-neutral-950 border border-neutral-800/80 rounded-3xl p-8 space-y-6 shadow-2xl backdrop-blur-xl">
      <div className="text-center space-y-2">
        ...
        <p className="text-xs text-neutral-400">
          Enter your registered email to access your profiles.
        </p>
      </div>
      ...
      <div className="pt-2 text-center text-xs text-neutral-500">
        Don’t have a card or profile yet?{' '}
        <Link href="/onboarding" className="text-white font-semibold hover:underline">
          Sign Up
        </Link>
      </div>
    </div>
  ...
  return (
    <main className="min-h-screen bg-black text-white font-sans selection:bg-neutral-800 flex flex-col justify-between p-6 relative overflow-hidden">
      ...
      <div className="my-auto flex justify-center py-6">
        <Suspense fallback={<div className="text-xs text-neutral-500 font-mono">Loading login interface...</div>}>
          <LoginForm />
        </Suspense>
      </div>
